---
layout: prediction
theme: crypto-depin
theme_title: "Crypto & DePIN"
date: 2023-09-25
year: "2023"
title: "Bringing AI to crypto is imperative, not optional"
permalink: /predictions/crypto-depin/2023-09-25-bringing-ai-to-crypto-is-imperative-not-optional-adn4/
slug_id: 2023-09-25-bringing-ai-to-crypto-is-imperative-not-optional-adn4
theme_page: /predictions/crypto-depin/
source: "Greg Osuri, Founder and CEO of Akash Network (Proof of Coverage)"
source_url: https://podcasts.apple.com/us/podcast/greg-osuri-founder-and-ceo-of-akash-network/id1645459705?i=1000629088449
timestamp: "00:30:37"
vid: pod-proof-of-coverage-greg-osuri-2023
quote: |
  So I think crypto is going to play a critical role and people are going to realize that sooner than later. And now we have an incredible opportunity. AI is a new industry and a new industry that's post crypto. So we have to bring AI to crypto. I don't think it's an option. It's more imperative at this point.
context: |
  His closing thesis, after mapping the decentralized ML stack: Gensyn (training), BitTensor (incentivized evals), data marketplaces (IP-safe training data), provenance.
---
