---
layout: prediction
theme: local-compute
theme_title: "Local Compute"
date: 2025-08-02
year: "2025"
title: "22 million homes — \"the way we live may change\""
permalink: /predictions/local-compute/2025-08-02-22-million-homes-the-way-we-live-may-change-hrxi/
slug_id: 2025-08-02-22-million-homes-the-way-we-live-may-change-hrxi
theme_page: /predictions/local-compute/2025/
speaker: "Greg Osuri"
speaker_status: "attributed"
source: "Solving the AI energy crisis | Greg Osuri on what it takes to power AI (Changelog)"
source_url: https://www.youtube.com/watch?v=61OOQfY5cqU&t=3716s
timestamp: "01:01:56"
vid: 61OOQfY5cqU
quote: |
  We have access to 22 million homes right now through our partners in the US... Not just sell them, but actually deploy. I think the way we live may change, don't you think?
context: |
  Host jokes xAI's 50M GPUs would only need 25M homes at two GPUs each; Greg also says a consumer home product is ~a year out, his own home will be the first, and a few homes will run within 6 months via Star Cluster.
---
